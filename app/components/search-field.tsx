"use client";

import { Icon } from "@iconify/react";
import { useRouter } from "next/navigation";
import { useEffect, useRef, useState } from "react";

/**
 * The search box. It owns the input, the URL owns the results.
 *
 * Typing replaces the history entry rather than pushing, so the back button
 * leaves the search page instead of walking letter by letter through what you
 * typed. The query lives in `?q=` so a result set can be linked and reloaded.
 */
export function SearchField({ initialQuery }: { initialQuery: string }) {
  const router = useRouter();
  const [value, setValue] = useState(initialQuery);
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    // Debounced: at ~30 keystrokes a query, a request per keystroke would be
    // 30 searches to answer one question.
    const id = setTimeout(() => {
      const next = value.trim();
      if (next === initialQuery) return;
      router.replace(next ? `/search?q=${encodeURIComponent(next)}` : "/search");
    }, 250);

    return () => clearTimeout(id);
  }, [value, initialQuery, router]);

  return (
    <div className="relative">
      {/* Same icon the header uses, so the control reads as the same feature. */}
      <Icon
        icon="iconamoon:search"
        width={22}
        height={22}
        aria-hidden
        className="pointer-events-none absolute top-1/2 left-4 -translate-y-1/2 text-bone-dim"
      />

      <input
        ref={inputRef}
        type="search"
        // autoFocus is right here: arriving at /search means you came to type.
        autoFocus
        value={value}
        onChange={(event) => setValue(event.target.value)}
        placeholder="Search by title…"
        aria-label="Search films by title"
        className="h-14 w-full rounded-lg border border-ink-line bg-bone/8 pr-12 pl-13 text-lg text-bone transition-colors placeholder:text-bone-dim focus:border-lamp focus:outline-none [&::-webkit-search-cancel-button]:hidden"
      />

      {value.length > 0 && (
        <button
          type="button"
          onClick={() => {
            setValue("");
            inputRef.current?.focus();
          }}
          aria-label="Clear search"
          className="absolute top-1/2 right-3 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-full text-bone-dim transition-colors hover:bg-bone/10 hover:text-bone"
        >
          <Icon icon="lucide:x" width={18} height={18} aria-hidden />
        </button>
      )}
    </div>
  );
}
