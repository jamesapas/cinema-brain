...
export default function Loading() {
  return (
    <main className="flex-1 pb-24">
      {/* Hero section skeleton — exact same height, background overlays, and structure as real MoviePage */}
      <section className="relative isolate min-h-[62vh] w-full overflow-hidden bg-ink sm:min-h-[70vh]">
        ...
        <div className="page-container relative flex min-h-[62vh] flex-col justify-end pt-24 pb-12 sm:min-h-[70vh] lg:pb-16">
          <div className="max-w-3xl">
            {/* Title */}
            <div className="skeleton h-7 w-3/4 rounded sm:h-10 lg:h-12" />

            {/* Meta (score · year · runtime · genres) */}
            <div className="mt-2 skeleton h-5 w-80 max-w-full rounded sm:mt-3" />

            {/* Tagline */}
            <div className="mt-2 skeleton h-4 w-60 max-w-full rounded sm:mt-3" />
            ...
            {/* Action bar (Ask Kino, Trailer, Watchlist & Favorite icons, Divider, Star Rating) */}
            <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-3">
              {/* Ask Kino button skeleton */}
              <div className="skeleton h-10 w-[7.5rem] rounded-md" />
              {/* Trailer button skeleton */}
              <div className="skeleton h-10 w-24 rounded-md" />
              {/* List buttons (Watchlist & Favorite) skeletons */}
        ...
        <div className="no-scrollbar flex gap-4 overflow-hidden pb-3">
          {[...Array(12)].map((_, i) => (
            ...
            >
              ...
              <div className="mt-0.5 skeleton h-3.5 w-12 rounded sm:h-4 sm:w-14" />
            </div>
          ))}
        </div>
      </div>
